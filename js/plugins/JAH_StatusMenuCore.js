...
//=============================================================================
/*:
 * @plugindesc v1.02 Creates an advanced Status window with multiple pages for each Actor.  Supports profiles and portraits.
 * @author JahwsUF
 *
...
JAH_Window_Status_Params.prototype.getLineCount = function(portraitMode) {
	...
	return maxCol + JAH_Window_Status_Base.prototype.getLineCount.call(this);
}
...
// Implements a Status page that displays a single Actor's parameters.
//
// This (in its present form) is probably the best example window to show others
// to expand upon.
...
JAH_Window_Status_Profile.prototype.getLineCount = function() {
	...
	totalLines += JAH_Window_Status_Base.prototype.getLineCount.call(this);
...
JAH_Window_Status_Profile.prototype.printScrolledText = function(text, x, y, scroll) {
	...
	for(var i = 0; i < lines.length; i++)
	{
		if(scroll >= 0)
		{
			if(lines[i].trim() == "<hr>")
			{
				this.drawHorzLine(y, this.availableWidth());
			}
			else this.drawTextEx(lines[i], x, y);
			y += lineHeight;
		}
